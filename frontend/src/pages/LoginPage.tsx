...
import { Leaf, Globe, ShieldCheck, Loader2 } from 'lucide-react';
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Access Denied",
        description: "Invalid credentials for the Guardian portal.",
        variant: "destructive"
      });
    }
  };
          ...
          <div>
            ...
            >
              ...
              <p className="text-emerald-50/60 text-xl font-medium leading-relaxed">
                Welcome back to India's premier environmental oversight platform. Access your cultivation protocols and regional growth metrics in real-time.
              </p>
            </motion.div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-10">
            <div className="space-y-4">
               <div className="flex items-center gap-3 text-emerald-400">
                  <Globe size={24} />
                  <span className="text-sm font-black uppercase tracking-widest">Global Coverage</span>
               </div>
               <p className="text-white/40 text-xs font-bold leading-relaxed uppercase tracking-widest">Active nodes across 28 states ensuring biological integrity.</p>
            </div>
            <div className="space-y-4">
               <div className="flex items-center gap-3 text-emerald-400">
                  <ShieldCheck size={24} />
                  <span className="text-sm font-black uppercase tracking-widest">Verified Growth</span>
               </div>
               <p className="text-white/40 text-xs font-bold leading-relaxed uppercase tracking-widest">Certified carbon-capture metrics protected by elite encryption.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-10 xl:px-32 py-20 relative bg-white">
        <div className="max-w-md w-full mx-auto space-y-12">
          <div className="space-y-4">
            ...
            <h1 className="text-4xl lg:text-5xl font-black text-slate-800 tracking-tighter leading-none">Authentication</h1>
            <p className="text-slate-400 font-medium text-lg italic uppercase tracking-wider text-[10px] b-4">Secure Portal • Level 4 Access</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-6">
              <div className="space-y-3 pt-4">
                ...
                <Input 
                  ...
                  className="h-14 bg-slate-50 border-none rounded-2xl px-6 text-lg focus-visible:ring-primary/20 ring-offset-0 transition-all font-medium placeholder:text-slate-200 placeholder:opacity-40"
                />
              </div>
            ...
            <Button 
               ...
               disabled={loading}
            >
              {loading ? <Loader2 className="animate-spin" /> : "SYNCHRONIZE CORE"}
            </Button>
          </form>

          <p className="text-center text-slate-400 font-bold text-xs uppercase tracking-[0.2em]">
            New Guardian? <Link to="/register" className="text-primary hover:underline ml-2">Initiate Training</Link>
          </p>
        </div>
