...
/// Handler for the Geospatial service.
pub async fn geospatial_handler(
    State(state): State<Arc<AppState>>,
    Extension(claims): Extension<Claims>,
    req: Request,
) -> impl IntoResponse {
    let upstream = state.geospatial_url.clone();
    match proxy_to(state, Some(claims), req, &upstream).await {
        Ok(r) => r.into_response(),
        Err(e) => e.into_response(),
    }
...
/// Handler for the Telemetry service.
pub async fn telemetry_handler(
    State(state): State<Arc<AppState>>,
    Extension(claims): Extension<Claims>,
    req: Request,
) -> impl IntoResponse {
    let upstream = state.telemetry_url.clone();
    match proxy_to(state, Some(claims), req, &upstream).await {
        Ok(r) => r.into_response(),
        Err(e) => e.into_response(),
    }
...
/// Handler for the Carbon service.
pub async fn carbon_handler(
    State(state): State<Arc<AppState>>,
    Extension(claims): Extension<Claims>,
    req: Request,
) -> impl IntoResponse {
    let upstream = state.carbon_url.clone();
    match proxy_to(state, Some(claims), req, &upstream).await {
        Ok(r) => r.into_response(),
        Err(e) => e.into_response(),
    }
